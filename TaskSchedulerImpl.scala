  ...
  extends TaskScheduler with Logging
{
  ...
  //keke:2019-4-20
  //CoarseGrainedSchedulerBackend需要传参数！！！
  //var nbackend: CoarseGrainedSchedulerBackend = null
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
	  ...
	  logInfo(s"KEKE hasReceivedTask true")
    }
  ...
  private def resourceOfferSingleTaskSet(
	...
	logInfo(s"KEKE execute resourceOfferSingleTaskSet")
    return launchedTask
  }
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    for (taskSet <- sortedTaskSets) {
     ...
     logInfo(s"KEKE execute resourceOffers")
  ...
  //keke:2019-4-18
    //定义全局变量nls存储已分配资源的任务
  val nls = new ArrayBuffer[AddCollectionTask]
  //keke:2019-4-18
  ...
  def addTask(tasks:Seq[Seq[TaskDescription]]){
        for(ntask <- tasks.flatten){
			...
			var preTime: Int = 0
			if(executorIdToHost(nexecId) == "192.168.1.104") preTime = 9
			if(executorIdToHost(nexecId) == "192.168.1.105") preTime = 7
			if(executorIdToHost(nexecId) == "192.168.1.106") preTime = 8
			var nstate: TaskState = TaskState.LAUNCHING
			val ls = new AddCollectionTask(ntId, nexecId, preTime, nstate)
			nls += ls
